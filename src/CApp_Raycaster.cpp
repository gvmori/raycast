...
double CApp::FindDistance(
    ...
    // Vector2<double> B;
    // if (absolute_angle > 90 && absolute_angle < 270) {
    //     B.x = (int)(pos->x / grid_size) * grid_size - 1; 
    // }
    // else {
    //     B.x = (int)(pos->x / grid_size) * grid_size + grid_size; 
    // }

    // B.y = pos->y + ((pos->x - B.x) * A_tan);
    // std::cout << "px: " << pos->x << " Bx: " << B.x << " py: " << pos->y << " By: " << B.y << std::endl;

    // // TODO: clean this up!
    // // refactor so the code can be used on both horiz&vert?
    // bool found_v_wall = false;
    // Vector2<int> v_wall;
    // if (level_array[(int)(B.x/grid_size)][(int)(B.y/grid_size)]){
    //     v_wall.x = (int)B.x;
    //     v_wall.y = (int)B.y;
    //     found_v_wall = true;
    // }
    ...
    //     Vector2<int> iterdiff;
    //     // TODO: precalculate or cache
    //     iterdiff.y = (grid_size/abs_tan);
    //     iterdiff.x = grid_size;
    //     if (absolute_angle > 90 && absolute_angle < 270) {
    //         iterdiff.x = iterdiff.x * -1;
    //     }
    ...
    //         B.x += iterdiff.x;
    //         B.y += iterdiff.y;
    //         std::cout << "Bx: " << B.x << " By: " << B.y << " By mod: " << (int)(B.y/grid_size) <<  std::endl;

    //         if (level_array[(int)(B.x/grid_size)][(int)(B.y/grid_size)]) {
    //             std::cout << "wall vert found at: " << (int)(B.x/grid_size) << "," << (int)(B.y/grid_size) << std::endl;
    //             v_wall.x = B.x;
    //             v_wall.y = B.y;
    //             found_v_wall = true;
    //         } 
    ...
    // return std::min(
    // double distx = (pos->x - wall.x) / cos(DegToRad(absolute_angle));
    // if (distx < 0) { distx = distx * -1; }
    // double disty = (pos->y - v_wall.y) / sin(DegToRad(absolute_angle));
    // if (disty < 0) { disty = disty * -1; }
    // int disty = abs((int)((pos->y - wall.y) / sin(DegToRad(absolute_angle))));
